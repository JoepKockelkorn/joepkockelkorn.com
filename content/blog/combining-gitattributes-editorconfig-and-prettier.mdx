...
import InfoCallout from '../../src/components/info-callout.astro'
...
<InfoCallout title="Disclaimer">
	Discuss and/or tweak the settings however you want, but whatever you do, use
	and enforce **a single standard**. Not everyone has to agree with it, but
...
For YAML files I set an exception for the `indent_style`. YAML files do not support tabs, so I set it to `space`.
...
```json
{
	"plugins": [
		...
		"prettier-plugin-tailwindcss" // if you use Tailwind
	],
	"singleQuote": true,
	"semi": false,
	"bracketSpacing": true,
...
Let's go through it. First, I set the `plugins` option to include the following plugins:

- [`@prettier/plugin-xml`](https://www.npmjs.com/package/@prettier/plugin-xml): Prettier does not format xml files out of the box.
- [`prettier-plugin-organize-imports`](https://www.npmjs.com/package/prettier-plugin-organize-imports): This sorts, combines and removes unused imports. Because nobody likes a messy import section.
- [`prettier-plugin-tailwindcss`](https://www.npmjs.com/package/prettier-plugin-tailwindcss): If you use Tailwind, this plugin will format the Tailwind classes in your HTML-like files.
...
- experimentalTernaries to `true`: This makes sure that ternaries are formatted in a more readable way (and will probably become the default way to format ternaries).
- overrides for specific files:
  - For JSONC files I set `trailingComma` to `none` because I've ran into issues with those.
  - For any `web.config` XML files (used in .NET development) I set `xmlWhitespaceSensitivity` to `ignore` so Prettier can format the whitespace.

<InfoCallout title="Prettier follows EditorConfig">
	You might be missing the `printWidth`, `tabWidth` or `useTabs` options in my
	Prettier config. But I omitted them on purpose, because Prettier will respect
	the settings from EditorConfig. And because EditorConfig applies to more files
	it's better to have the settings in EditorConfig and let Prettier use them.
</InfoCallout>
