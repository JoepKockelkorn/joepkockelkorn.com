...
// Copied from: https://github.com/xiefucai/marked-admonition-extension/blob/main/src/index.ts
/*
                                 Apache License
...
*/
// Changes applied: changed type imports to direct imports, fixed return type bug in tokenizer
import type { MarkedExtension, RendererExtension, RendererThis, TokenizerExtension, TokenizerThis, Tokens } from 'marked';
type Config = {
	nodeName: string;
	className: string;
...
const admonitionPlugin: TokenizerExtension | RendererExtension = {
    ...
    <${config.title.nodeName} class="${config.className}-title">${this.parser.parseInline(token.titleTokens, null as any)}</${
...
export default { extensions } satisfies MarkedExtension;
